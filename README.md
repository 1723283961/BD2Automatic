...
* [X] 末日之书自动刷分
* [ ] 自动跑图（目前完成第一章的脚本錄製（只能跑，不能夠空格加速，但加了個脚本錄製，支持自定義錄製脚本使用 ： 路徑/GameUtils/ScripRecordingTool/下的三個），等我後面慢慢跑完在上傳吧）
* [ ] UI脚本制作（制作ing）
* [ ] 兑换码快捷兑换
...
### License
本项目代码可用于 **学习 / 研究 / 个人用途**。  
除作者本人（[1723283961]）外，**禁止任何形式的商业使用**。

本项目依赖的 [OpenCV](https://opencv.org/) 遵循 Apache License 2.0，其使用需遵循原始开源协议。  
